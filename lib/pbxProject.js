...
pbxProject.prototype.removeFromPbxHeadersBuildPhase = function(file) {
  ...
  for (i in headers.files) {
      if (headers.files[i].comment == longComment(file, 'Headers')) {
...
function longComment(file, group) {
    return f("%s in %s", file.basename, group || file.group);
}
...
pbxProject.prototype.getPBXVariantGroupByKey = function(key) {
    if (!Object.keys(this.hash.project.objects).includes('PBXVariantGroup')) {
        return false;
    }
